...
const addProdutoDB = async (body) => {
    try {
        ...
        returning codigo,nome, descricao, quantidade_estoque, ativo, valor, 
...
const updateProdutoDB = async (body) => {
    try {
        ...
        WHERE codigo = $1 returning codigo,nome, descricao, quantidade_estoque, 
        ativo, valor,  to_char(data_cadastro, 'YYYY-MM-DD') as data_cadastro, 
        categoria`,
        ...
        throw "Erro ao alterar a produto: " + err;
    }
}
...
const deleteProdutoDB = async (codigo) => {
    try {
        ...
        if (results.rowCount == 0) {
            ...
            return `Produto de código ${codigo} removido com sucesso!`;
        }
    } catch (err) {
